...
async def stats_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """
    Показывает статистику по всем отправленным сигналам.
    Считается независимо от того, входил ли пользователь в сделку —
    это честный показатель качества сигналов бота.

    /stats       — за последние 30 дней
    /stats 7     — за последние 7 дней
    /stats 90    — за последние 90 дней
    """
    user_id = update.effective_user.id
    user = storage.get_user(user_id)
    ...
    if context.args:
        try:
            ...
            await update.message.reply_text("Укажи число дней от 1 до 365, например: /stats 7")
            return

    try:
        st = storage.get_signal_stats(user_id, days)
    except Exception as e:
        logger.error(f"stats_command: ошибка: {e}", exc_info=True)
        await update.message.reply_text("Не удалось получить статистику — ошибка базы.")
        return

    total = st["total"]
    if total == 0:
        await update.message.reply_text(
            f"📊 За последние {days} дней сигналов ещё нет.\n\n"
            f"Статистика накапливается автоматически — просто подожди."
        )
        return

    tp = st["tp"]; sl = st["sl"]; open_ = st["open"]
    closed = tp + sl
    winrate = tp / closed * 100 if closed > 0 else 0

    lines = [f"📊 *Статистика сигналов за {days} дней*\n"]
    lines.append(f"Всего сигналов: {total}")
    lines.append(f"✅ TP достигнут: {tp} ({tp/total*100:.0f}%)")
    lines.append(f"❌ SL достигнут: {sl} ({sl/total*100:.0f}%)")
    if open_ > 0:
        lines.append(f"⏳ Ещё не закрыты: {open_}")
    if closed > 0:
        lines.append(f"\n🎯 Винрейт (закрытые): *{winrate:.0f}%*")
    ...
    if by_s:
        lines.append("\n*По стратегии:*")
        strat_names = {
            "TREND_UP": "Тренд вверх ↗️",
            "TREND_DOWN": "Тренд вниз ↘️",
            "RANGE": "Боковик ↔️",
        }
        for strat, d in sorted(by_s.items()):
            name = strat_names.get(strat, strat)
            c = d["tp"] + d["sl"]
            wr = f"{d['tp']/c*100:.0f}%" if c > 0 else "—"
            lines.append(f"  {name}: {d['tp']+d['sl']+d['open']} сигн., винрейт {wr}")

    # По направлению
    by_d = st.get("by_direction", {})
    if by_d:
        lines.append("\n*По направлению:*")
        for dir_, d in sorted(by_d.items()):
            emoji = "🟢" if dir_ == "LONG" else "🔴"
            c = d["tp"] + d["sl"]
            wr = f"{d['tp']/c*100:.0f}%" if c > 0 else "—"
            lines.append(f"  {emoji} {dir_}: {d['tp']+d['sl']+d['open']} сигн., винрейт {wr}")

    lines.append("\n_Статистика считается по всем сигналам бота,_")
    lines.append("_вне зависимости от того, входил ли ты в сделку._")

    # --- Раздел КАНАЛ (отдельно от лички бота) ---
    try:
        ch = storage.get_channel_stats(days)
        if ch["total"] > 0:
            ch_closed = ch["win"] + ch["loss"]
            ch_wr = ch["win"] / ch_closed * 100 if ch_closed > 0 else 0
            lines.append("\n\n📢 *Канал ONE SIGNAL:*")
            lines.append(f"Опубліковано: {ch['total']}")
            lines.append(f"✅ Ціль 1: {ch['win']}")
            lines.append(f"🛑 Стоп: {ch['loss']}")
            if ch["open"] > 0:
                lines.append(f"⏳ Ще відкриті: {ch['open']}")
            if ch_closed > 0:
                lines.append(f"🎯 Вінрейт каналу: *{ch_wr:.0f}%*")
        else:
            lines.append("\n\n📢 *Канал:* сигналів ще не було")
    except Exception as e:
        logger.warning(f"stats_command: не удалось получить статистику канала: {e}")
...
def main():
    ...
    # --- Теневой лог: фиксируем TP/SL всех сигналов (каждые 5 минут) ---
    job_queue.run_repeating(
        scheduler.check_signal_log,
        interval=300,
        first=60,
    )
    # --- Канал: строгий отбор лучших сигналов (каждый час) ---
    job_queue.run_repeating(
        scheduler.scan_channel,
        interval=60 * 60,
        first=90,
    )
    # --- Канал: тихо фиксируем результат (Цель 1 / Стоп) каждые 5 минут ---
    job_queue.run_repeating(
        scheduler.check_channel_signals,
        interval=300,
        first=120,
    )
